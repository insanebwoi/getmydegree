...
export function WhatsAppButton() {
  ...
  const phone = '918606677828'
  const message = 'Hello GetMyDegree, I would like to know more about degree admission and counseling.'
  const href = `https://wa.me/${phone}?text=${encodeURIComponent(message)}`

  return (
    <aside
      aria-label="Contact options"
      className="fixed right-5 bottom-5 z-90 flex items-center gap-2.5 sm:right-6 sm:bottom-6"
    >
      {/* Tooltip / Label */}
      <span
        className={`hidden rounded-full bg-navy-950/90 px-3.5 py-1.5 text-xs font-medium text-white shadow-md backdrop-blur-sm transition-all duration-200 sm:inline-block ${
          hovered ? 'translate-x-0 opacity-100' : 'translate-x-1 opacity-0 pointer-events-none'
        }`}
      >
        Chat on WhatsApp
      </span>
      ...
      <a
        ...
        className="group relative flex h-13 w-13 items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_4px_20px_rgba(37,211,102,0.45)] transition-all duration-200 hover:scale-108 hover:bg-[#20ba5a] hover:shadow-[0_6px_25px_rgba(37,211,102,0.6)] focus:outline-none focus:ring-4 focus:ring-[#25D366]/30 sm:h-14 sm:w-14"
      >
        {/* Pulse effect ring */}
        <span
        ...
        {/* WhatsApp SVG Icon — official brand paths */}
        <svg
