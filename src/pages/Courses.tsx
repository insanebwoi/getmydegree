  ...
  return (
    <Reveal delay={delay}>
      <article className="card card-hover flex h-full flex-col p-6">
        ...
        <h3 className="mt-4 font-display text-lg font-medium">{course.name}</h3>
        <p className="mt-1 text-sm font-medium text-gold-700">{course.field}</p>
        <p className="mt-3 flex-1 text-sm leading-relaxed text-muted">{course.body}</p>
  ...
  return (
    <>
        ...
        <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {courseHighlights.map((h, i) => (
            <Reveal key={h} delay={i * 60} as="li">
              <div className="flex h-full items-center gap-4 rounded-[var(--radius-card)] border border-white/12 bg-white/5 p-5">
      ...
      <section className="shell pb-16 lg:pb-24">
        <Reveal>
          <div className="card flex flex-col items-center gap-6 p-9 text-center lg:flex-row lg:justify-between lg:p-12 lg:text-left">
            <div className="max-w-xl">
              <h2 className="text-2xl sm:text-3xl">Not sure which program suits you?</h2>
